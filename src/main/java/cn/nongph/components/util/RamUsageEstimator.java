 ...
 * 
 * <p>
 * NOTE: Starting with Lucene 3.6, creating instances of this class is
 * deprecated. If you still do this, please note, that instances of
 * {@code RamUsageEstimator} are not thread-safe! It is also deprecated to
 * enable checking of String intern-ness, the new static method no longer allow
 * to do this. Interned strings will be counted as any other object and count
 * for memory usage.
 * 
 * <p>
 * In Lucene 3.6, custom {@code MemoryModel}s were completely removed. The new
 * implementation is now using Hotspot&trade; internals to get the correct scale
 * factors and offsets for calculating memory usage.
 * 
 * @see #sizeOf(Object)
...
public final class RamUsageEstimator {
	...
	public static enum JvmFeature {
		OBJECT_REFERENCE_SIZE(
				"Object reference size estimated using array index scale"), ARRAY_HEADER_SIZE(
	 ...
	 * Objects will always take a full multiple of this constant, possibly
	 * wasting some space.
	 */
	...
	/**
	 * Initialize Constant and try to collect information about the JVM
	 * internals.
	 */
	static {
		...
		int referenceSize = SystemConstants.JRE_IS_64BIT ? 8 : 4;
		int objectHeader = SystemConstants.JRE_IS_64BIT ? 16 : 8;
		// The following is objectHeader + NUM_BYTES_INT, but aligned (object
		// alignment)
		// so on 64 bit JVMs it'll be align(16 + 4, @8) = 24.
		int arrayHeader = SystemConstants.JRE_IS_64BIT ? 24 : 12;
		...
		try {
			final Method arrayIndexScaleM = unsafeClass.getMethod(
		...
		objectHeader = SystemConstants.JRE_IS_64BIT ? (8 + referenceSize) : 8;
		arrayHeader = SystemConstants.JRE_IS_64BIT ? (8 + 2 * referenceSize) : 12;

		// get the object header size:
		// - first try out if the field offsets are not scaled (see warning in
		// Unsafe docs)
		// - get the object header size by getting the field offset of the first
		// field of a dummy object
		// If the scaling is byte-wise and unsafe is available, enable dynamic
		// size measurement for
		// estimateRamUsage().
		Method tempObjectFieldOffsetMethod = null;
		try {
			final Method objectFieldOffsetM = unsafeClass.getMethod(
					"objectFieldOffset", Field.class);
			final Field dummy1Field = DummyTwoLongObject.class
					.getDeclaredField("dummy1");
			final int ofs1 = ((Number) objectFieldOffsetM.invoke(theUnsafe,
					dummy1Field)).intValue();
			final Field dummy2Field = DummyTwoLongObject.class
					.getDeclaredField("dummy2");
			final int ofs2 = ((Number) objectFieldOffsetM.invoke(theUnsafe,
					dummy2Field)).intValue();
			if (Math.abs(ofs2 - ofs1) == NUM_BYTES_LONG) {
				final Field baseField = DummyOneFieldObject.class
		...
		try {
			final Method arrayBaseOffsetM = unsafeClass.getMethod(
					"arrayBaseOffset", Class.class);
			// we calculate that only for byte[] arrays, it's actually the same
			// for all types:
		...
		try {
			final Class<?> beanClazz = Class
					.forName("com.sun.management.HotSpotDiagnosticMXBean");
			final Object hotSpotBean = ManagementFactory
			...
			objectAlignment = Integer.parseInt(vmOption.getClass()
					.getMethod("getValue").invoke(vmOption).toString());
		...
		JVM_INFO_STRING = "[JVM: " + SystemConstants.JVM_NAME + ", "
				+ SystemConstants.JVM_VERSION + ", " + SystemConstants.JVM_VENDOR + ", "
				+ SystemConstants.JAVA_VENDOR + ", " + SystemConstants.JAVA_VERSION + "]";
	}
	 ...
	 * {@code RamUsageEstimator}. If this method returns {@code false} you are
	 * maybe using a 3rd party Java VM that is not supporting Oracle/Sun private
	 * APIs. The memory estimates can be imprecise then (no way of detecting
	 * compressed references, alignments, etc.). Lucene still tries to use
	 * sensible defaults.
	 */
	...
	public static long sizeOf(char[] arr) {
		return alignObjectSize((long) NUM_BYTES_ARRAY_HEADER
				+ (long) NUM_BYTES_CHAR * arr.length);
	}

	/** Returns the size in bytes of the short[] object. */
	public static long sizeOf(short[] arr) {
		return alignObjectSize((long) NUM_BYTES_ARRAY_HEADER
				+ (long) NUM_BYTES_SHORT * arr.length);
	}

	/** Returns the size in bytes of the int[] object. */
	public static long sizeOf(int[] arr) {
		return alignObjectSize((long) NUM_BYTES_ARRAY_HEADER
				+ (long) NUM_BYTES_INT * arr.length);
	}

	/** Returns the size in bytes of the float[] object. */
	public static long sizeOf(float[] arr) {
		return alignObjectSize((long) NUM_BYTES_ARRAY_HEADER
				+ (long) NUM_BYTES_FLOAT * arr.length);
	}

	/** Returns the size in bytes of the long[] object. */
	public static long sizeOf(long[] arr) {
		return alignObjectSize((long) NUM_BYTES_ARRAY_HEADER
				+ (long) NUM_BYTES_LONG * arr.length);
	}

	/** Returns the size in bytes of the double[] object. */
	public static long sizeOf(double[] arr) {
		return alignObjectSize((long) NUM_BYTES_ARRAY_HEADER
				+ (long) NUM_BYTES_DOUBLE * arr.length);
	}

	/**
	 * Estimates the RAM usage by the given object. It will walk the object tree
	 * and sum up all referenced objects.
	 * 
	 * <p>
	 * <b>Resource Usage:</b> This method internally uses a set of every object
	 * seen during traversals so it does allocate memory (it isn't side-effect
	 * free). After the method exits, this memory should be GCed.
	 * </p>
	 */
	 ...
	 * would occupy. This works with all conventional classes and primitive
	 * types, but not with arrays (the size then depends on the number of
	 * elements and varies from object to object).
	 * 
	 * @see #shallowSizeOf(Object)
	 ...
	 * ~5000 objects linked in a chain so not too much).
	 */
	private static long measureObjectSize(Object root, boolean checkInterned) {
		...
		while (!stack.isEmpty()) {
			...
			// *** BEGIN deprecation
			if (checkInterned && ob instanceof String
					&& ob == ((String) ob).intern()) {
				continue;
			}
			// *** END deprecation
				...
				try {
					...
					if (cachedInfo == null) {
						classCache.put(obClazz,
								cachedInfo = createCacheEntry(obClazz));
					}
	...
	/**
	 * Create a cached information about shallow size and reference fields for a
	 * given class.
	 */
	 ...
	 * 
	 * <p>
	 * The returned offset will be the maximum of whatever was measured so far
	...
	private static long adjustForField(long sizeSoFar, final Field f) {
		...
		if (objectFieldOffsetMethod != null) {
			try {
				final long offsetPlusSize = ((Number) objectFieldOffsetMethod
						.invoke(theUnsafe, f)).longValue() + fsize;
				return Math.max(sizeSoFar, offsetPlusSize);
			} catch (IllegalAccessException ex) {
				throw new RuntimeException(
						"Access problem with sun.misc.Unsafe", ex);
			...
			// TODO: No alignments based on field type/ subclass fields
			// alignments?
	...
	public static String humanReadableUnits(long bytes) {
		return humanReadableUnits(bytes, new DecimalFormat("0.#",
	 ...
	 * allowed.
	 * 
	 * TODO: If this is useful outside this class, make it public - needs some
	 * work
	 */
	static final class IdentityHashSet<KType> implements Iterable<KType> {
		 ...
		 * 
		 * <p>
		 * The implementation is based on the finalization step from Austin
		 * Appleby's <code>MurmurHash3</code>.
		 * 
		...
		/**
		 * Expand the internal storage buffers (capacity) or rehash current keys
		 * and values if there are a lot of deleted slots.
		 */
	 ...
	 *             {@link #sizeOf(Object)} method that has no intern checking,
	 *             too.
	 */
	@Deprecated
	 ...
	 * 
	 * @param checkInterned
	 *            check if Strings are interned and don't add to size if they
	 *            are. Defaults to true but if you know the objects you are
	 *            checking won't likely contain many interned Strings, it will
	 *            be faster to turn off intern checking. Intern checking is
	 *            deprecated altogether, as it is not free of side-effects and
	 *            strains the garbage collector additionally.
	 * @deprecated Don't create instances of this class, instead use the static
